...
//变量声明
let g_lpf_cm_install_seccomp_filter = null, g_lpf_cm_thread_syscall_tvar, g_lpf_cm_call_task, g_lpf_cm_lock, g_lpf_cm_unlock, g_lpf_cm_findSoinfoByAddr, solist_get_head_ptr, get_soname, g_lpg_cm_get_base, g_lpf_cm_get_size, maps = [];

const MAX_STACK_TRACE_DEPTH = 10;//栈回溯深度
const Target_NR =  56; //被监控的系统调用
//__NR_recvfrom 207
//__NR_openat 56
...
function hook_init() 
{
    ...
    //CModule层,函数获取
    g_lpf_cm_thread_syscall_tvar = new NativeFunction(cm.pthread_syscall_create, "pointer", [])()  //创建一个没有被过滤的线程
...
//这些函数地址都会通过 Module.findExportByName 获取
...
extern void on_message(const gchar *message);//js层,console.log输出
...
void *call_syscall(void *args)
{
    ...
    //参数传递多了,不影响??
    // syscall(rax,rdi,rsi,rdx,rcx,r8,r9) 刚好6个参数 从寄存器获取,而不是stack
...
void *call_task(thread_syscall_t *syscall_thread,void *args,int type)
{
    ...
    do
    {
        //如果 syscall_thread 不满足情况回导致死循环在这里???
...
thread_syscall_t *pthread_syscall_create()
{
    ...
    syscall_thread->isTask = 0; //一开始线程内部无任务
    syscall_thread->args = NULL;//一开始线程内部无参数
    syscall_thread->ret = NULL;//一开始线程内部无返回值
    syscall_thread->isReturn = 0;//一开始线程内部未返回
    pthread_mutex_init(&syscall_thread->mutex, NULL); //互斥体初始化
    pthread_t threadId;
    pthread_create(&threadId, NULL, &pthread_syscall, (void *)syscall_thread);//创建线程,线程参数是一个可被外部访问的互斥体
...
int install_seccomp_filter(__u32 nr) //目前函数是支持监控1个syscall调用
{
    log("[*] install_seccomp_filter(%lu)",nr);
    struct sock_filter filter[] = 
    {
            BPF_STMT(BPF_LD + BPF_W + BPF_ABS, 0),
            BPF_JUMP(BPF_JMP + BPF_JEQ + BPF_K, nr, 0, 1), //nr是我们要监控的系统调用
            BPF_STMT(BPF_RET + BPF_K, SECCOMP_RET_TRAP), //发现并监控
            BPF_STMT(BPF_RET + BPF_K, SECCOMP_RET_ALLOW), //其它允许调用
    };
...
function utils_hex(arrayBuffer) 
{
...
function utils_call_thread_log(str) 
{
    g_lpf_cm_call_task(g_lpf_cm_thread_syscall_tvar, Memory.allocUtf8String(str), 1)
}

function utils_call_thread_read_maps(addr) 
{
    ...
    const map_info = g_lpf_cm_call_task(g_lpf_cm_thread_syscall_tvar, ptr(addr), 2).readUtf8String()
    //7ff7075000-7ff7874000 rw-p 00000000 00:00 0 [stack]
...
function utils_addrToString(addr) 
{
    const add_s = parseInt(addr.toString(10))
    const addr_soinfo = g_lpf_cm_findSoinfoByAddr(ptr(add_s));
    if (addr_soinfo != 0) 
    {
        return `0x${addr.toString(16)}[${get_soname(addr_soinfo).readUtf8String()}:0x${(addr - g_lpg_cm_get_base(addr_soinfo)).toString(16)}]`
    }
    ...
    const mem_region = utils_call_thread_read_maps(add_s);
    if (mem_region.name != "") 
    {
...
function utils_stacktrace(pc, fp, sp) //堆栈回溯
{
    let n = 0, stack_arr = [], fp_c = fp;
    stack_arr[n++] = pc;
    const mem_region = utils_call_thread_read_maps(sp);
