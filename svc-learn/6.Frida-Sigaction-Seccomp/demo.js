let g_lpf_cm_mm_install_signalhandle = null;
...
function init()
{
    g_lpf_cm_mm_install_signalhandle = new NativeFunction(cm.mm_install_signalhandle, "void", []); //拿到CModule的函数
    g_lpf_cm_mm_install_signalhandle();
}
...
function hook_dlopen()
{
    ...
    Interceptor.attach(android_dlopen_ext, 
    {
        onEnter: function(args) {
            if (g_lpf_cm_mm_install_signalhandle == null) 
            {
                init()
            }
        },
...
#define log(...) __android_log_print(3, "ECHO", __VA_ARGS__)
...
void mm_install_signalhandle(){
    ...
    sa.sa_sigaction = mm_sig_handler;
    sa.sa_mask = sigset;
